...
module "tls_automagically" {
  ...
  ca_common_name    = "${format("%s", resource.random_pet.env.id)}.local"
  dns_names = concat(
  ...
  target_regions        = toset(local.regions)
...
resource "helm_release" "kube_prometheus_stack" {
  ...
  #   value = "changeme"
  # }
  # values = [templatefile("${path.root}/config/kube-prometheus-stack.tftpl", {
  #   custom_dashboards = fileset("${path.root}/custom_dashboards/", "*.json"),
  #   module_path       = path.root
  # })]
  values = [templatefile("${path.module}/config/kube-prometheus-stack.tftpl", {
